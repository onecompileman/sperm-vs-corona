class GameManager {
  ...
  initScreenManager() {
    ...
    this.screenManager.menuScreen
      .querySelector('#howToBtn')
      .addEventListener('click', () => {
        this.screenManager.howToPlay.style.display = 'flex';
      });

    this.screenManager.menuScreen
      .querySelector('#about')
      .addEventListener('click', () => {
        this.screenManager.about.style.display = 'flex';
      });

    this.screenManager.howToPlay
      .querySelector('.close-button')
      .addEventListener('click', () => {
        this.screenManager.howToPlay.style.display = 'none';
      });

    this.screenManager.about
      .querySelector('.close-button')
      .addEventListener('click', () => {
        this.screenManager.about.style.display = 'none';
      });
  ...
  initCorona() {
    this.coronaGenerationTime = 12 * 60;
  ...
  initEnemyFollow() {
    ...
    this.enemyFollowIncreaseDiffRate = 30 * 60;
    this.enemyFollowIncreaseDiff = this.enemyFollowIncreaseDiffRate;

    this.enemyFollowGenerationTime = 8 * 60;
    this.enemyFollowGeneration = 0;
    this.enemyFollowMinGenerationTime = 6 * 60;
    this.enemyFollowMaxGenerationTime = 10 * 60;
  ...
  renderEnemiesShoot() {
    this.increaseEnemyShootDifficulty();
    this.generateEnemyShoot();

    this.enemiesShoot = this.enemiesShoot.filter((enemy, i) => {
      const bullet = enemy.fire();

      if (bullet) {
        this.enemyBullets.push(bullet);
        if (this.soundManager.enemyShoot.isPlaying()) {
          this.soundManager.enemyShoot.stop();
        }
        this.soundManager.enemyShoot.setVolume(0.1);
        this.soundManager.enemyShoot.play();
      }

      enemy.follow(this.sperm);
      enemy.update();
      enemy.stayAwayToOtherEnemies([
        ...this.enemiesFollow,
        ...this.enemiesShoot
      ]);
      enemy.display();
